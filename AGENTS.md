...
4. **post_process** — 第二次扫描 `tables/`，顺序执行：rename 目录（基于叠加层信息的安全网）→ 移 orphan → 并行写 `tables/tables.json` + `indexes/*.json` → 写 `tables/state.toml`（审计记录）
...
- **reconcile 默认流水线中不做**——rename 在内联在三个位置：(1) overlay 阶段用 `compute_renames`（基于磁盘 `info.json`）预重命名，确保 fetch 写入正确命名的目录；(2) **fetch 阶段**内部用 `compute_prefetch_dir_name` 做基于叠加层信息的预重命名（在 HTTP 请求前，即使 fetch 失败也执行）；(3) post_process 用 `compute_renames_overlaid`（基于叠加层信息而非磁盘 `info.json`）做安全网重命名，避免撤销 fetch 阶段的预重命名。`reconcile` 子命令仅手动调用时使用
